...
I will be using the train set to build 3 classifier algorithms, let them predict on the test set to check out who is the best  classifier. 
...
mean(predicted.classes.knn == test.set$Termd)
...
```
* Sensitivity is only 42% which is the proportion of terminated staff correctly identified as terminated.

```{r}

predicted.classes.knn.prob <- model_knn %>% predict(test.set, type = "prob")

res.roc.knn <- roc(test.set$Termd, predicted.classes.knn.prob[,1])
plot.roc(res.roc.knn, print.auc = T)
```

```{r}
res.roc.knn <- roc(test.set$Termd, predicted.classes.knn.prob[,2])
plot.roc(res.roc.knn, print.auc = T)
