...
### 8.5 we predict who is going to terminate and who isn't? What level of accuracy can we achieve on this?
...
g2 <- ggplot(df_term, aes(x = years_worked)) +
  ...
  theme_bw() +
  labs(title = "Numbers of Years that Terminated Employees have worked") +
...
g3 <- ggplot(df, aes(y = fct_reorder(TermReason, count), x = count, fill = EmploymentStatus)) +
  ...
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title.y = element_blank()) +
  labs(title = "Top Reasons Terminated Employees Left") +
  geom_text(aes(label = count), hjust = -0.3) +
   facet_wrap(~EmploymentStatus, scale = "free", nrow = 2, ncol = 1)
...
plot_grid(top, middle, g6, ncol = 1)
...
ggplot(df_term, aes(x = years_worked)) +
  geom_histogram(color = "black", fill = "purple") +
  theme_bw() +
  labs(title = "Numbers of Years that Terminated Employees have worked") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
...
```{r}
df8.5.2 <- hr2 %>% 
   select(Salary, Termd, Position, Age, Sex, MaritalDesc, CitizenDesc, HispanicLatino, RaceDesc, years_worked, Department, ManagerName, RecruitmentSource, PerformanceScore, EngagementSurvey, EmpSatisfaction, SpecialProjectsCount, DaysLateLast30, Absences)
